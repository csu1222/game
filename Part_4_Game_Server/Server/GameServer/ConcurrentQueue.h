...
public:
	...
	void Push(const T& value)
	{
		...
		// 새로 추가할 데이터를 유니크 포인터로 만든다 
		unique_ptr<T> newData = make_unique<T>(value);

		// 기존 dummy에 새로운 데이터 덮어 씌우고 그 뒤에 붙일 새 dummy를 만듭니다. 
		CountedNodePtr dummy;
		dummy.ptr = new Node;
		dummy.externalCount = 1;

		CountedNodePtr oldTail = _tail.load();	// ptr = nullptr 인상태 

		while (true)
		{
			// 참조권 획득 (externalCount를 현시점 기중 +1 한 애가 이김)
