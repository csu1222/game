...
9. 문제 : 궁수가 죽으면 펫도 같이 죽는 사양이 추가 되었습니다. 그런데 왜 프로그램도 같이 죽을까요?

	solution) 궁수가 공격을 받아서 죽었다면 펫도 delete해주는 사양입니다. 어디가 문제일지 디버깅을 실행해보니 프로그램 종료때 궁수 객체를 delete
	하는 부분에서 크래시가 됐습니다. 그래서 궁수의 소멸자를 살펴보니 여기에서도 펫을 삭제하고 있습니다. 데미지 판정으로 궁수가 죽을때도 펫을 삭제
	하고 프로그램 종료시에도 삭제하고 있습니다. 이중 삭제가 된겁니다. 소멸자에서 펫이 널포인트인지 체크를 해주고 있지만 유효하지 않았습니다.
	데미지 체크시에 펫을 삭제하였지만 _pet자체는 힙영역에 있는 객체를 가리키는 주소 바구니고 펫을 삭제하면 주소를 타고간 그 메모리를 할당해제 한
	것입니다. 즉 _pet이라는 포인터의 주소자체는 그냥 유효하지않은 메모리라도 가리키고는 있다는 겁니다. 그래서 동적할당을 삭제 할때는 delete후 
	포인터를 nullptr로 초기화 해주어야 겠습니다.

10. 문제 : 궁수라면 자고로 화살을 쏴야겠죠. Arrow는 자신의 목표대상물을 target으로 관리하고 Arrow를 만들때 설정한 값만큼의 공격력을 가지게 됩니다.
	테스트를 위해 화살을 10개 생성해서 기사를 10번 쐈는데 크래시가 났습니다.

	solution) 프로그램에서 화살을 포인터 배열로 10의 크기를 가지게 만들어 첫번째 인자으로 타겟을 주고 두번째인자로 공격력을 주는 객체를 각각 생성
	해주었습니다. 다시 반복문으로 0번 인덱스의 화살부터 하나씩 기사를 공격한후 기사를 null체크도하고 죽음체크도 해서 죽었으면 기사를 삭제후 nullptr
	할당까지 해주었습니다. 마지막으로 공격한 화살객체도 삭제해주었습니다. 이 코드를 쭉 디버깅하다보면 타겟을 쏜후 타겟의 정보를 출력하는 코드에서 
	읽기 엑세스위반, this->_target이 0xDDDDDDDDDDD이었다고 합니다. 메모리 값이 이상한것이 이미 해제된 메모리 같습니다.타겟은 기사라는 객체를 
	가리키고 있었고 화살공격 반복중에 기사가 죽어서 삭제후 nullptr로 밀어져 버리면 남은 화살들의 target이었던 주소가 유효하지 않게 되어 버립니다.
	에초에 기사를 삭제할 때 남은 화살들에게 타겟이 이제 유효하지 않다고 알려줘야 합니다. 그리고 에러가 난 코드가 PrintInfo였고 그 위에 AddHp가 
	있는데 AddHp에서는 왜 에러가 안나고 그냥 넘어가고 PrintInfo에서 에러가 났을까 보면 AddHp는 _hp라는 자리의 값에 value만큼 더해주는 함수고 
	PrintInfo는 기사 클래스에서 가상함수로 만들어 놨습니다. 가상함수 테이블로 기사의 원본을 추적해서 실행하는데 기사의 PrintInfo를 실행하면 부모의 
	PrintInfo함수의 주소가 아닌 기사의 PrintInfo주소로 접근해서 정보를 출력하려고 하는데 이미 밀린 주소라서 에러가 난것 같습니다.
	해결방법으로 화살을 전부 쏠때까지 기사의 삭제를 미루고 모든 과정이 지나고 나서 기사를 삭제해주면 되겠습니다.
*/
