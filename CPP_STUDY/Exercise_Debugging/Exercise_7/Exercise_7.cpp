...
/*
크래쉬가 나는 이유는 Archer객체에서 생성한 pet객체가 Archer가 delete할때 Archer 소멸자 -> Player 소멸자 순
으로 호출이 되야 하는데 바로 Player 소멸자부터 호출이 되었습니다. 그래서 Pet객체는 delete되지 않고 Heap메모리에
계속 누수 되었습니다.

virtual ~Player()로 가상 소멸자 함수로 만들어 Archer , 
Player순으로 소멸자를 호출 하도록 하였습니다.
*/
int main()
{
